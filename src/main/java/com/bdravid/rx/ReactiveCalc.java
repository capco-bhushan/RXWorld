 ...
 */
public class ReactiveCalc {
    private final static Scanner scanner = new Scanner(System.in);
    public static void main(String[] args) throws InterruptedException {
        PublishSubject<String> numberStream = PublishSubject.create();
        numberStream.subscribe(new Logger());
        Adder adder = new Adder();
        adder.subscribe(new Average());
        numberStream.subscribe(adder);

        Observable<String> observable = Observable.generate(stringEmitter -> stringEmitter.onNext(scanner.next()));
        observable.subscribe(numberStream);
    }
